...
-- Toggle function: if pointed node is dirt, convert to grass;
-- if it is grass, convert to dirt.
powers.toggle_dirt_grass = function(itemstack, user, pointed_thing)
...
powers.whirlwind = function(itemstack, user, pointed_thing)
    ...
    for pointed in ray do
        if pointed.type == "object" and pointed.ref and pointed.ref:is_player() and 
           pointed.ref:get_player_name() ~= caster_name then
            target = pointed.ref
            break
        end
    end
...
powers.dig_tunnel = function(itemstack, user, pointed_thing)
    ...
    local function dig_next(idx)
        ...
        if not core.is_protected(pos, player_name) then
            ...
            if minetest.registered_nodes[node_name] and minetest.registered_nodes[node_name].groups.unbreakable ~= 1 then
    ...
    if node_below == "default:dirt_with_grass" then
        for i = 1, 2 do
            ...
            if not minetest.is_protected(flower_pos, player:get_player_name()) then
                ...
                if (ground_node == "default:dirt" or ground_node == "default:dirt_with_grass") and
                   minetest.get_node(flower_pos).name == "air" and #flower_list > 0 then
...
local wand = {
    ...
    on_use_sneak = powers.toggle_dirt_grass,
