--api by TenPlus1
local S = core.get_translator("mymagic_mobs")
local FS = function(...) return core.formspec_escape(S(...)) end
local use_cmi = core.global_exists("cmi")
local use_mc2 = core.get_modpath("mcl_core")
...
minetest.register_node("mymagic_mobs:fallback_node", {
...
mymagic_mobs = {
	...
	node_dirt = has(core.registered_aliases["mapgen_dirt"])
			or has("mcl_core:dirt") or has("default:dirt") or "mymagic_mobs:fallback_node"
}
mymagic_mobs.fallback_node = mymagic_mobs.node_dirt
...
function mymagic_mobs.is_creative(name)
...
local mob_class = mymagic_mobs.mob_class -- Compatibility
...
function mymagic_mobs:yaw(entity, yaw, delay) -- [deprecated]
...
function mymagic_mobs:set_animation(entity, anim) -- [deprecated]
...
function mymagic_mobs:line_of_sight(entity, pos1, pos2) -- [deprecated]
...
function mymagic_mobs:yaw_to_pos(self, target, rot) -- [deprecated]
...
function mymagic_mobs:effect(pos, amount, texture, min_size, max_size, radius, grav, glow, fall)
...
function mymagic_mobs:remove(self, decrease)
...
local function node_ok(pos, fallback)
	...
	return core.registered_nodes[(fallback or mymagic_mobs.fallback_node)]
end

function mymagic_mobs:node_ok(pos, fallback)
...
function mymagic_mobs:is_node_dangerous(mob_object, nodename)
...
function mymagic_mobs:is_invisible(self, player_name)
		...
		for n = 1, #objs do
			...
			and ent.horny and ent.hornytimer <= HORNY_TIME then
				...
				core.after(5, function(self, ent)
					...
					-- add baby
					local ent2 = mymagic_mobs:add_mob(pos, {
						name = self.name,
						child = true,
						owner = self.owner,
						ignore_count = true
				...
				if p1.y > (s.y + 1) then
					...
					if not core.is_protected(s, "") then
						...
						if ndef1 and (ndef1.buildable_to or ndef1.groups.liquid) then
							core.set_node(s, {name = mymagic_mobs.fallback_node})
						end
					end
...
core.register_entity("mymagic_mobs:_pos", {
	initial_properties = {
...
function mob_class:go_to(pos)
	...
	local obj = core.add_entity(pos, "mymagic_mobs:_pos")
...
--print("=== explosion timer", self.timer)
				...
				if self.timer > self.explosion_timer then
					...
					mymagic_mobs:boom(self, pos, node_break_radius, entity_damage_radius)
...
function mob_class:on_punch(hitter, tflp, tool_capabilities, dir, damage)
	...
	if not tool_capabilities then
		core.log("warning",	"[mymagic_mobs] Mod profiling enabled, damage not enabled")
		return true
	end
...
--print("Mob Damage is", damage)
	...
	if mymagic_mobs.is_creative(hitter:get_player_name()) then
...
function mymagic_mobs:register_mob(name, def)

	mymagic_mobs.spawning_mobs[name] = {}
...
function mymagic_mobs:can_spawn(pos, name)
	return can_spawn(pos, name)
end

-- global functions

function mymagic_mobs:add_mob(pos, def)
...
--print("--- no position or definition given")
	...
	if not mymagic_mobs.spawning_mobs[def.name] or not core.registered_entities[def.name] then
...
--print("--- no players within active area, will not spawn " .. def.name)
	...
	local aoc = mymagic_mobs.spawning_mobs[def.name] and mymagic_mobs.spawning_mobs[def.name].aoc or 1
...
function mymagic_mobs:spawn_abm_check(pos, node, name)
	-- return true to stop spawning mob
end

-- older spawning function

function mymagic_mobs:spawn_specific(name, nodes, neighbors, min_light, max_light, interval,
		chance, aoc, min_height, max_height, day_toggle, on_spawn, map_load)

	-- Do mobs spawn at all?
	if not mobs_spawn or not mymagic_mobs.spawning_mobs[name] then
--print ("--- spawning not registered for " .. name)
	...
	if numbers then
		...
		if chance == 0 then
			...
			core.log("warning",
					string.format("[mymagic_mobs] %s has spawning disabled", name))
			return
		end

		core.log("action", string.format(
				"[mymagic_mobs] Chance setting for %s changed to %s (total: %s)",
				name, chance, aoc))
	end

	mymagic_mobs.spawning_mobs[name].aoc = aoc
...
--print("-- lbm no chance or neighbor not found")
		...
		-- is mob actually registered?
		if not mymagic_mobs.spawning_mobs[name] or not ent then
...
--print("--- active mob limit reached", active_mobs, active_limit)
		...
		if mymagic_mobs:spawn_abm_check(pos, node, name) then
			return
		end
...
--print("--- mob needs day", name)
		...
		if #core.find_nodes_in_area(
				...
				{x = pos.x + 16, y = pos.y + 16, z = pos.z + 16},
				{"mymagic_mobs:mob_repellent"}) > 0 then
...
--print ("--- inside block", name, node_ok(pos2).name)
		...
		if pos then
			...
			if mob_log_spawn then
				...
				core.log("[mymagic_mobs] Spawned " .. (name or "") .. " at " .. pos_string)
			end
...
function mymagic_mobs:register_spawn(name, nodes, max_light, min_light, chance,
		active_object_count, max_height, day_toggle)

	mymagic_mobs:spawn_specific(name, nodes, {"air"}, min_light, max_light, 30,
		chance, active_object_count, -31000, max_height, day_toggle)
end

-- MarkBu's newer spawn function (USE this one please modders)

function mymagic_mobs:spawn(def)

	mymagic_mobs:spawn_specific(
		def.name,
...
function mymagic_mobs:register_arrow(name, def)
...
function mymagic_mobs:safe_boom(self, pos, radius, texture)
...
function mymagic_mobs:boom(self, pos, node_damage_radius, entity_radius, texture)
	...
	and not core.is_protected(pos, "") then
		...
		mymagic_mobs:safe_boom(self, pos, node_damage_radius, texture)
	end
end

function mymagic_mobs:explosion(pos, radius) -- [deprecated] compatibility function
	mymagic_mobs:boom({sounds = {explode = "tnt_explode"}}, pos, radius, radius, "tnt_smoke.png")
end
...
function mymagic_mobs:register_egg(mob, desc, background, addegg, no_creative)
	...
	core.register_craftitem(":" .. mob, {
		...
		on_place = function(itemstack, placer, pointed_thing)
			...
			if pos and not core.is_protected(pos, placer:get_player_name()) then
				...
				if not mymagic_mobs.is_creative(placer:get_player_name()) then
					itemstack:take_item()
				end
			end
...
function mymagic_mobs:force_capture(self, clicker)
...
function mymagic_mobs:capture_mob(
	...
	and tool:get_name() ~= "mymagic_mobs:net" and tool:get_name() ~= "mymagic_mobs:lasso" then
		return false
	end
	...
	if clicker:get_inventory():room_for_item("main", mobname) then
		...
		elseif tool:get_name() == "mymagic_mobs:net" then
		...
		elseif tool:get_name() == "mymagic_mobs:lasso" then
...
function mymagic_mobs:protect(self, clicker)

	local name = clicker:get_player_name()
	local tool = clicker:get_wielded_item()
	local tool_name = tool:get_name()

	if tool_name ~= "mymagic_mobs:protector" and tool_name ~= "mymagic_mobs:protector2" then
		return false
	end
	...
	if (self.protected and tool_name == "mymagic_mobs:protector")
	or (self.protected == 2 and tool_name == "mymagic_mobs:protector2") then
		core.chat_send_player(name, S("Already protected!")) ; return true
	end

	if not mymagic_mobs.is_creative(clicker:get_player_name()) then
		tool:take_item() -- take 1 protection rune
		clicker:set_wielded_item(tool)
	end

	-- set protection level
	if tool_name == "mymagic_mobs:protector" then
		self.protected = true
	else
...
function mymagic_mobs:feed_tame(self, clicker, feed_count, breed, tame)
...
--print ("====", self.hornytimer)
	...
	if item:get_name() == "mymagic_mobs:nametag"
...
core.register_on_player_receive_fields(function(player, formname, fields)
	...
	if formname == "mobs_nametag" and fields.name and fields.name ~= "" then
		...
		if item:get_name() ~= "mymagic_mobs:nametag" then return end
		...
		-- if not in creative then take item
		if not mymagic_mobs.is_creative(name) then
...
function mymagic_mobs:alias_mob(old_name, new_name)
